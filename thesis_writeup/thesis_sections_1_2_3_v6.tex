...
The implementation reflects this observation directly.  A \texttt{ZnEncodingMap} stores:
\begin{itemize}[leftmargin=1.5em]
...
The output classifier is a \texttt{PLEncodingMapBoxes}.  Besides the region signatures and representatives, it stores the coordinate axes, the mixed-radix cell shape and strides, and a cell-to-region table.  As in the lattice case, this supports fast repeated point location.  When a point lies away from a cell boundary, location is a direct mixed-radix lookup.  Near a boundary, the implementation can either use a fast convention or a more careful verified mode, depending on verification requirements.

\subsubsection*{The general PL/polyhedral backend}

Axis-aligned boxes occur often enough that they merit their own specialized backend, but they are not the only natural real parameter front end.  In many situations the relevant generators and cogenerators are fully polyhedral: they are cut out by arbitrary linear inequalities and therefore meet along oblique facets rather than along coordinate hyperplanes.  The ambient parameter space is then divided by a finite collection of polyhedral boundaries, and the resulting constant regions of the module need no longer be rectangular or even combinatorially simple.  The general PL backend is designed for exactly this richer setting.

The first implementation decision is to represent each convex piece by an explicit halfspace description
\[
A x \leq b
\]
stored over the exact coefficient field \( \Q \).  This is important because the encoder needs an exact description of the geometry while it is deciding which signatures are actually realized, and floating-point approximations may be too inexact. Additionally, we make the design choice where point membership tests should not depend on a particular version of an external polyhedral package in \texttt{Julia} like \texttt{Polyhedra} or \texttt{CDDLib}. So, the basic polyhedral object stores the \(H\)-representation itself, and routine membership in a convex piece or a finite union of such pieces is evaluated directly from the stored inequalities.
...
\]
that are actually realized by points of \(\R^n\), where \(y_i\) records membership in \(U_i\) and \(z_j\) records non-membership in \(D_j\).  The naive search space is still finite, but potentially large, so the implementation is organized around a branch-and-feasibility strategy that only keeps signatures that are geometrically realizable.
...
The general PL backend is also the route by which the library supports \emph{common encoding} of several PL fringe presentations at once.  In that case the upsets and downsets from all inputs are pooled, feasibility enumeration is performed once on the combined family, and the resulting common region poset is then sliced back into one pushed-down fringe presentation for each input module.  This is a substantial advantage of the general backend over the box-specialized route: once several modules have been encoded into the same finite poset, later comparisons such as Hom, Ext, and invariant-level distances can be carried out directly on a common finite model.

Finally, the PL backend is also where region geometry begins to enter the picture.  Because the encoding map stores actual polyhedral regions, the library can later ask geometric questions about those regions inside a finite window: their volume, bounding box, centroid, boundary measure, adjacency pattern, or principal directions.  To make repeated geometry queries tractable, the backend provides a dedicated \texttt{PolyInBoxCache}.  This cache stores, lazily and relative to a fixed ambient box, the intersections of regions with that box, their exact or approximate \(V\)- and \(H\)-representations, active-region masks, axis-aligned bounding boxes, facet data, and exact weights or centroids when these have already been computed.  Thus the geometric layer is not an afterthought added after encoding.  It is a natural extension of the same compiled region structure and will later serve as the prelude to weighted invariants, feature extraction, and visualization.

For all of these reasons, the general PL backend should be viewed as more than merely ``the slow but general one.''  Its role is to make finite encoding over \(\R^n\) mathematically correct even when the front-end geometry is genuinely polyhedral, while still remaining tractable through a careful separation of concerns: exact arithmetic for region construction, lightweight halfspace tests for repeated queries, signature-first enumeration rather than full arrangement construction, and cache-driven reuse for point location and region geometry.
  

\subsection{Geometry attached to an encoding map}
\label{subsec:encoding-geometry}

The encoding step produces more than just a finite poset and a classifier.  It also produces a finite stratification of the ambient parameter space.  Once this stratification exists, one may ask geometric questions about its regions.
...
This geometric layer should be understood correctly.  It is not another pre-encoding input type.  The regions whose geometry is being measured are not the original front-end generators, but the strata produced by the encoding itself.  In other words, region geometry is an attribute of the encoded decomposition of parameter space, not of the initial presentation data.

That distinction matters because the later invariant layer uses exactly this sort of information.  Weighted Hilbert summaries, support measures, adjacency-based summaries, shape descriptors, and region-based visualizations all depend on geometric data attached to the encoding map.  The present chapter therefore introduces the idea, while the later chapter on invariants will use it systematically.

From the implementation point of view, the division is again backend-sensitive.  For lattice encodings and box-based real encodings, many geometric quantities can be read off directly from the critical-coordinate lists and the region lookup tables.  For more general PL encodings, the same questions require a richer geometric interface.  The codebase isolates those geometric queries in a separate layer so that invariant and visualization routines can ask for region weights, boundary size, or adjacency without having to know which backend supplied them.

\subsection{From filtered data to finite encodings}
\label{subsec:data-to-encoding}

The front ends discussed so far begin with explicit presentations.  The data-ingestion layer begins earlier, with filtered data.  Its task is to convert raw data into a finite algebraic object that can be encoded by the same workflow.
...
The presence of the simplex-tree route is especially important.  It means that the pipeline does not need to materialize every intermediate presentation eagerly in order to reach an encoded module.  When the input is simplicial, the packed simplex tree can be kept long enough for the boundary and cohomology computations to be built lazily.  This is one of the main reasons the ingestion layer scales better than a naive expand-everything approach.

At the same time, the endpoint is the same as for the presentation-based encoders.  Once the cohomology module on the finite grid poset has been built, it is packaged as an encoded object with a classifier map.  Thus filtered data reaches the same finite-poset world as flanges and PL fringes do, only by a different route.

This is also the right place to clarify what it means to encode \emph{from a graded module}.  If the user already has a finite labeled cell complex or a module supported on finitely many grades, then the set of grades itself determines finite axes.  The encoding problem is then not one of discovering signatures from infinite geometry, but of building the finite grid poset that makes those grades reusable as a module object and equips them with a classifier map and workflow metadata.  In this sense, data-derived modules and presentation-derived modules meet at the same conceptual endpoint: a finite encoded module on a finite poset.

\subsection{Common encodings, compiled evaluation, and reusable results}
...
The other side of reuse is repeated evaluation.  Once an encoding has been built, later invariant and visualization routines may need to locate thousands or millions of ambient parameter values.  A purely formal encoding map would be enough for correctness, but it would not be enough for performance.  The library therefore compiles encoding maps into wrappers that cache axes, representatives, and hot-path metadata, and that provide repeated evaluation routines such as batched point location.

This is why the workflow distinguishes between an abstract encoding map and a compiled encoding.  The abstract object records the mathematics; the compiled object records the mathematics together with the cached data needed to make repeated evaluation cheap.  The same distinction appears again in the result layer.  An \texttt{EncodingResult} is not just a tuple of mathematical objects, but a reusable workflow object designed to survive later algebra, invariant computation, serialization, and benchmarking.

\subsection{Complexity, diagnostics, and the role of the encoding chapter}

A fully formal complexity theory for the entire encoding layer would be long and not especially illuminating, because the dominant costs depend strongly on the front end.  Nevertheless, the main cost centers are easy to identify.

For flange presentations over $\Z^n$, the dominant tasks are extracting critical coordinates, traversing slab cells, deduplicating realized signatures, building the finite signature poset, and pushing the presentation to that poset.  For box presentations over $\R^n$, the story is similar, except that the ambient cells are geometric rather than purely discrete.  For general PL presentations, region construction and point location are more expensive because the cell structure is no longer rectangular.  For data ingestion, the main additional costs are filtration construction, boundary assembly or traversal, and cohomology computation before the encoded module is even packaged.

The most important practical failure mode is region explosion.  If too many critical thresholds are present, the induced cell structure can become too large to be useful.  The library therefore exposes construction budgets, axis coarsening policies, and backend-selection diagnostics.  Other diagnostics concern degenerate boundaries, unsupported backend combinations, and stage-specific restrictions, such as the requirement that flange output from data ingestion only makes sense when the final axes are integral.

These diagnostics are not peripheral.  They are part of the value of making the encoding step explicit.  A library that treats encoded modules as its central internal object must also explain when such an object can be built cheaply, when the geometry is too large, and which backend assumptions are being used.  The encoding layer is therefore both the conceptual heart of the library and the place where its computational promises are either realized or fail.

From this point onward, the thesis works almost entirely inside the category of modules over a finite encoding poset.
