...
# This file is intended to be included from test/runtests.jl.
#
# Important:
# - Do NOT `import PosetModules` as a package here. When the test suite is run by
#   include("test/runtests.jl"), the module is typically loaded as Main.PosetModules
#   (not necessarily installed in the current Julia environment).
# - We therefore grab whichever PosetModules is already in scope.
...
"""
    simple_pmodule(P, v) -> IR.PModule{QQ}
...
"""
    interval_pmodule(P, a, b) -> IR.PModule{QQ}
...
function direct_sum(mods::Vector{IR.PModule{QQ}})
    ...
    return IR.PModule{QQ}(Q, dims, edge_maps)
end

"""
    endo_at_vertex(M, u, A) -> IR.PMorphism{QQ}
...
"""
    compose_morphism(g, f) -> IR.PMorphism{QQ}
...
@testset "Model-independent Ext layer: comparison + coherent transport" begin
    ...
    E = PM.Ext(M, N; maxdeg = 1, model = :unified, canon = :projective)
...
@testset "Injective model functoriality in second argument" begin
    ...
    Einj = PM.ExtInjective(M, N; maxdeg = 2)
...
@testset "Ext long exact sequence in first argument (degree 0 connecting map)" begin
    ...
    i = IR.PMorphism{QQ}(S2, I12, i_comps)

    p_comps = [zeros(QQ, S1.dims[v], I12.dims[v]) for v in 1:Q.n]
    p_comps[1] = ones(QQ, 1, 1)
    p = IR.PMorphism{QQ}(I12, S1, p_comps)

    les = PM.ExtLongExactSequenceFirst(S2, I12, S1, S2, i, p; maxdeg = 0)

    # Connecting map delta: Ext^0(S2,S2) -> Ext^1(S1,S2) should be nonzero (rank 1).
    @test PM.rankQQ(les.deltaH[1]) == 1
    @test size(les.deltaH[1]) == (PM.dim(les.EC, 1), PM.dim(les.EA, 0))
end
...
@testset "Injective chain-map lifting (public API)" begin
    ...
    g = IR.PMorphism{QQ}(N, N, [fill(QQ(2), 1, 1), fill(QQ(3), 1, 1)])

    res = PM.injective_resolution(N; maxlen=2)
