...
struct IndFlat
    ...
    function IndFlat(b::AbstractVector{<:Integer}, tau::Face, id::Symbol=:F)
        length(b) == tau.n || error("IndFlat: length(b) must match face dimension")
        return new(collect(Int, b), tau, id)
    end
end

"Indecomposable injective E = k[c + Z^tau - N^n]."
struct IndInj
    b::Vector{Int}
    tau::Face
    id::Symbol

    function IndInj(b::AbstractVector{<:Integer}, tau::Face, id::Symbol=:E)
        length(b) == tau.n || error("IndInj: length(b) must match face dimension")
        return new(collect(Int, b), tau, id)
    end
end
...
function canonical_matrix(flats::Vector{IndFlat}, injectives::Vector{IndInj})
    # The intersection pattern is purely combinatorial and does not depend on the
    # coefficient ring. Returning Int keeps it cheap to build and easy to convert.
    Phi = zeros(Int, length(injectives), length(flats))
    for q in 1:length(injectives), p in 1:length(flats)
        Phi[q, p] = intersects(flats[p], injectives[q]) ? 1 : 0
    end
    return Phi
end
