...
using ..IndicatorResolutions: PModule, PMorphism, map_leq
import ..IndicatorResolutions: pullback

using ..DerivedFunctors: projective_resolution, injective_resolution,
                         lift_chainmap, lift_injective_chainmap
...
function pushforward_left(pi::EncodingMap, f::PMorphism{QQ}; check::Bool=true)::PMorphism{QQ}
    if check
        _check_monotone(pi)
        if !_same_poset(pi.Q, f.dom.Q) || !_same_poset(pi.Q, f.codom.Q)
    ...
    cod_out, data_cod = _left_kan_data(pi, f.codom; check=false)
...
function pushforward_right(pi::EncodingMap, f::PMorphism{QQ}; check::Bool=true)::PMorphism{QQ}
    ...
    cod_out, data_cod = _right_kan_data(pi, f.codom; check=false)
...
"""
    pushforward_left_complex(pi, M; maxdeg=3, check=true, res=nothing)

Compute a cochain complex whose cohomology in degree -i is L_i pushforward_left(pi, M).

Speed: pass a precomputed `ProjectiveResolution` via `res` to avoid recomputation.
"""
function pushforward_left_complex(pi::EncodingMap, M::PModule{QQ};
                                  maxdeg::Int=3, check::Bool=true,
                                  res=nothing)
    if check
        _check_monotone(pi)
        if !_same_poset(pi.Q, M.Q)
...
"""
    Lpushforward_left(pi, M; maxdeg=3)

Return `[L_0, L_1, ..., L_maxdeg]` where `L_i = L_i pushforward_left(pi, M)`.
"""
function Lpushforward_left(pi::EncodingMap, M::PModule{QQ};
                           maxdeg::Int=3, check::Bool=true)
    C = pushforward_left_complex(pi, M; maxdeg=maxdeg, check=check)
...
derived_pushforward_left(pi::EncodingMap, M::PModule{QQ}; maxdeg::Int=3, check::Bool=true) =
    Lpushforward_left(pi, M; maxdeg=maxdeg, check=check)

"""
    pushforward_left_complex(pi, f; maxdeg=3, check=true, res_dom=nothing, res_cod=nothing)

Return a cochain map between the complexes `pushforward_left_complex(pi, f.dom)` and
`pushforward_left_complex(pi, f.codom)`.

The induced map on cohomology in degree `-i` is the map on left-derived functors
`L_i pushforward_left(pi, f)`.
...
"""
function pushforward_left_complex(pi::EncodingMap, f::PMorphism{QQ};
                                  maxdeg::Int=3, check::Bool=true,
                                  res_dom=nothing, res_cod=nothing)
    if check
        _check_monotone(pi)
        @assert _same_poset(pi.Q, f.dom.Q)
        @assert _same_poset(pi.Q, f.codom.Q)
    end

    maxlen = maxdeg + 1

    if res_dom === nothing
        res_dom = projective_resolution(f.dom; maxlen=maxlen)
    end
    if res_cod === nothing
        res_cod = projective_resolution(f.codom; maxlen=maxlen)
    end

    Cdom = pushforward_left_complex(pi, f.dom; maxdeg=maxdeg, check=false, res=res_dom)
    Ccod = pushforward_left_complex(pi, f.codom; maxdeg=maxdeg, check=false, res=res_cod)
...
"""
    Lpushforward_left(pi, f; maxdeg=3, check=true)

Return the induced maps on left-derived pushforward:

    out[i+1] : L_i pushforward_left(pi, f.dom) -> L_i pushforward_left(pi, f.codom)

for i = 0..maxdeg.
"""
function Lpushforward_left(pi::EncodingMap, f::PMorphism{QQ};
                           maxdeg::Int=3, check::Bool=true,
                           res_dom=nothing, res_cod=nothing)
    F = pushforward_left_complex(pi, f; maxdeg=maxdeg, check=check,
                                 res_dom=res_dom, res_cod=res_cod)
...
derived_pushforward_left(pi::EncodingMap, f::PMorphism{QQ}; maxdeg::Int=3, check::Bool=true) =
    Lpushforward_left(pi, f; maxdeg=maxdeg, check=check)


"""
    pushforward_right_complex(pi, M; maxdeg=3, check=true, res=nothing)

Compute a cochain complex whose cohomology in degree i is R^i pushforward_right(pi, M).

Speed: pass a precomputed `InjectiveResolution` via `res` to avoid recomputation.
"""
function pushforward_right_complex(pi::EncodingMap, M::PModule{QQ};
                                   maxdeg::Int=3, check::Bool=true,
                                   res=nothing)
    if check
        _check_monotone(pi)
...
"""
    Rpushforward_right(pi, M; maxdeg=3)

Return `[R^0, R^1, ..., R^maxdeg]` where `R^i = R^i pushforward_right(pi, M)`.
"""
function Rpushforward_right(pi::EncodingMap, M::PModule{QQ};
                            maxdeg::Int=3, check::Bool=true)
    C = pushforward_right_complex(pi, M; maxdeg=maxdeg, check=check)
    out = Vector{PModule{QQ}}(undef, maxdeg+1)
    for i in 0:maxdeg
        out[i+1] = cohomology_module(C, i)
    end
...
derived_pushforward_right(pi::EncodingMap, M::PModule{QQ}; maxdeg::Int=3, check::Bool=true) =
    Rpushforward_right(pi, M; maxdeg=maxdeg, check=check)
    
"""
    pushforward_right_complex(pi, f; maxdeg=3, check=true, res_dom=nothing, res_cod=nothing)

Return the induced cochain map between `pushforward_right_complex(pi, f.dom)` and
`pushforward_right_complex(pi, f.codom)`.
...
4. package as a `ModuleCochainMap`
"""
function pushforward_right_complex(pi::EncodingMap, f::PMorphism{QQ};
                                   maxdeg::Int=3, check::Bool=true,
                                   res_dom=nothing, res_cod=nothing)
    if check
        _check_monotone(pi)
    ...
    if res_dom === nothing
        res_dom = injective_resolution(f.dom; maxlen=maxlen)
    end
    if res_cod === nothing
        res_cod = injective_resolution(f.codom; maxlen=maxlen)
    end

    Cdom = pushforward_right_complex(pi, f.dom; maxdeg=maxdeg, check=false, res=res_dom)
    Ccod = pushforward_right_complex(pi, f.codom; maxdeg=maxdeg, check=false, res=res_cod)
...
"""
    Rpushforward_right(pi, f; maxdeg=3, check=true)

Return the induced maps on right-derived pushforward:

    out[i+1] : R^i pushforward_right(pi, f.dom) -> R^i pushforward_right(pi, f.codom)

for i = 0..maxdeg.
"""
function Rpushforward_right(pi::EncodingMap, f::PMorphism{QQ};
                            maxdeg::Int=3, check::Bool=true,
                            res_dom=nothing, res_cod=nothing)
    F = pushforward_right_complex(pi, f; maxdeg=maxdeg, check=check,
                                  res_dom=res_dom, res_cod=res_cod)
    out = Vector{PMorphism{QQ}}(undef, maxdeg + 1)
    for i in 0:maxdeg
        out[i+1] = induced_map_on_cohomology_modules(F, i)
...
derived_pushforward_right(pi::EncodingMap, f::PMorphism{QQ}; maxdeg::Int=3, check::Bool=true) =
    Rpushforward_right(pi, f; maxdeg=maxdeg, check=check)
