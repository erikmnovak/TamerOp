...
struct ExtSpaceProjective{K}
    # Domain and codomain modules.
    M::PModule{K}
    N::PModule{K}

    # Chosen projective resolution of M and the induced Hom complex Hom(P_*, N).
    res::ProjectiveResolution{K}
    complex::ChainComplexes.CochainComplex{K}

    # Per-degree offsets used to pack Hom(P_a, N) into a single coordinate vector.
    offsets::Vector{Vector{Int}}

    # Cached cohomology data for degrees 0..tmax (one entry per degree).
    cohom::Vector{Any}

    # Cached cochain degree range.
    tmin::Int
    tmax::Int
end
...
function Ext(res::ProjectiveResolution{QQ}, N::PModule{QQ})
    ...
    # Store tmin/tmax explicitly to avoid getproperty-based "virtual fields".
    return ExtSpaceProjective{QQ}(res.M, N, res, C, offs, cohom, C.tmin, C.tmax)
end
...
- `model = :unified` returns a model-independent
...
function ExtLongExactSequenceSecond(M::PModule{QQ},
    ...
    return ExtLongExactSequenceSecond{QQ}(0, maxdeg, EA, EB, EC, iH, pH, delta)
end
...
struct ExtSpaceInjective{K}
    ...
    # Chosen injective resolution of N and the induced Hom complex Hom(M, I^*).
    res::InjectiveResolution{K}

    # Component Hom spaces Hom(M, I^b) (one per injective degree b).
    homs::Vector{HomSpace{K}}

    complex::ChainComplexes.CochainComplex{K}
    cohom::Vector{ChainComplexes.CohomologyData{K}}

    # Cache tmin/tmax explicitly to avoid getproperty-based derived fields.
    tmin::Int
    tmax::Int
end
"""
...
function ExtInjective(M::PModule{QQ}, resN::InjectiveResolution{QQ})
    ...
    cohom = [ChainComplexes.cohomology_data(C, t) for t in 0:L]
    # Store tmin/tmax explicitly to avoid getproperty-based "virtual fields".
    return ExtSpaceInjective{QQ}(M, N, resN, homs, C, cohom, C.tmin, C.tmax)
end
...
function ExtLongExactSequenceFirst(
    ...
    delta = Vector{Matrix{QQ}}(undef, tmax - tmin + 1)
    ...
    return ExtLongExactSequenceFirst{QQ}(tmin, tmax, EA, EB, EC, pH, iH, delta)
end
...
The object contains:
- Tor spaces `TorA=Tor(Rop,A)`, `TorB=Tor(Rop,B)`, `TorC=Tor(Rop,C)` computed using ONE fixed resolution
...
- For mathematician-friendly access, we provide aliases:
    * `LES.TorA`, `LES.TorB`, `LES.TorC` as aliases for `LES.TorA`, `LES.TorB`, `LES.TorC`.
    * `LES.delta` as an alias for `LES.delta` (the stored connecting maps).
"""
struct TorLongExactSequenceSecond{K}
    ...
    # Tor spaces computed using one fixed projective resolution of Rop.
    TorA::TorSpace{K}
    TorB::TorSpace{K}
    TorC::TorSpace{K}

    # Maps in the long exact sequence (homological degrees s=0..maxdeg stored at index s+1).
    iH::Vector{Matrix{K}}
    pH::Vector{Matrix{K}}
...
struct TorLongExactSequenceFirst{K}
    ...
    # Tor spaces computed using one fixed projective resolution of L.
    ...
    # Connecting maps delta[s] : Tor_s(C,L) -> Tor_{s-1}(A,L) for s=1..maxdeg.
    delta::Vector{Matrix{K}}
