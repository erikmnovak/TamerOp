...
@inline _edge_maps_haskey(edge_maps::AbstractDict{Tuple{Int,Int},<:Any}, u::Int, v::Int) = haskey(edge_maps, (u, v))
@inline _edge_maps_getindex(edge_maps::AbstractDict{Tuple{Int,Int},<:Any}, u::Int, v::Int) = edge_maps[(u, v)]

@inline _edge_maps_haskey(edge_maps::CoverEdgeMapStore, u::Int, v::Int) = haskey(edge_maps, u, v)
@inline _edge_maps_getindex(edge_maps::CoverEdgeMapStore, u::Int, v::Int) = edge_maps[u, v]
...
`edge_maps` may be:
- a mapping keyed by `(u,v)` tuples (e.g. `Dict{Tuple{Int,Int},<:AbstractMatrix}`), or
- an existing `CoverEdgeMapStore` (in which case canonical `edge_maps[u,v]` access is used).
"""
function CoverEdgeMapStore{K,MatT}(
    ...
    for v in 1:n
        ...
        @inbounds for i in eachindex(pv)
            u = pv[i]
            if _edge_maps_haskey(edge_maps, u, v)
                A = convert(MatT, _edge_maps_getindex(edge_maps, u, v))
            else
