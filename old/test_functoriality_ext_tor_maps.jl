...
# This test file is included from runtests.jl and reuses the shared helpers/constants
# defined there:
#   - PM, FF, IR, QQ
#   - chain_poset, one_by_one_fringe
# Avoid redefining them here to prevent constant redefinition warnings and API drift.
...
function scalar_endo(M::IR.PModule{QQ}, s::QQ)
...
@testset "Tor functoriality in both arguments" begin
    ...
    T_R2_L = PM.Tor(R2, L; maxdeg=3)
    T_R2_L2 = PM.Tor(R2, L2; maxdeg=3)
...
@testset "TorLongExactSequenceSecond + TorAlgebra generator" begin
    ...
    i = IR.PMorphism{QQ}(A, B, [zeros(QQ, 1, 0), QQ[1;;],         zeros(QQ, 0, 0)])
    p = IR.PMorphism{QQ}(B, C, [QQ[1;;],         zeros(QQ, 0, 1), zeros(QQ, 0, 0)])
    ...
    LES = PM.TorLongExactSequenceSecond(Rop, i, p; maxdeg=2)

    TorRA = PM.Tor(Rop, A; maxdeg=2)
    TorRB = PM.Tor(Rop, B; maxdeg=2)
    TorRC = PM.Tor(Rop, C; maxdeg=2)

    @test LES.maxdeg == 2
    @test length(LES.iH) == 3
    @test length(LES.pH) == 3
    @test length(LES.deltaH) == 2
    ...
    T = PM.Tor(Rop, B; model=:second, maxdeg=2)
...
@testset "hyperTor_map_first/second: induced maps on Tor_n" begin
    ...
    C = PM.ModuleCochainComplex([L], IR.PMorphism{QQ}[]; tmin=0, check=true)

    HT = PM.hyperTor(Rop, C; maxlen=2)
    T  = PM.Tor(Rop, L; maxdeg=2)
...
@testset "Tor extra structure: LES, actions, bicomplex" begin
    ...
    i = IR.PMorphism{QQ}(S2, P1, [zeros(QQ, 1, 0), QQ[1;;]])
    p = IR.PMorphism{QQ}(P1, S1, [QQ[1;;], zeros(QQ, 0, 1)])

    les2 = PM.TorLongExactSequenceSecond(S2op, i, p; maxdeg=1)

    # Connecting map delta: Tor_1(S2op, S1) -> Tor_0(S2op, S2)
    # In this toy example it is nonzero (this is the standard non-split SES).
    @test size(les2.deltaH[1], 1) == 0
    @test size(les2.deltaH[2]) == (PM.dim(les2.TA, 0), PM.dim(les2.TC, 1))
    @test les2.deltaH[2][1, 1] != 0

    # Short exact sequence in the first variable: 0 -> S1op -> P2op -> S2op -> 0
    i1 = IR.PMorphism{QQ}(S1op, P2op, [QQ[1;;], zeros(QQ, 1, 0)])
    p1 = IR.PMorphism{QQ}(P2op, S2op, [zeros(QQ, 0, 1), QQ[1;;]])

    les1 = PM.TorLongExactSequenceFirst(S1, i1, p1; maxdeg=1)

    @test size(les1.deltaH[2]) == (PM.dim(les1.TA, 0), PM.dim(les1.TC, 1))
    @test les1.deltaH[2][1, 1] != 0

    # Ext action on Tor via the resolve-second model:
    # The Ext^0 unit should act as identity on Tor_1.
    EA = PM.ExtAlgebra(S1; maxdeg=2)
    Tsec = PM.Tor(S2op, S1; model=:second, res=EA.E.res)
    u = PM.unit(EA)
    ...
    Tfirst = PM.Tor(S2op, S1; maxdeg=2)
