...
function grid_poset(a::Vector{Int}, b::Vector{Int})
    ...
    # Materialize as concrete NTuple{n,Int}.  (The iterator returns a Tuple whose
    # static type can be hard to pin down when n is runtime-determined.)
...
"""
function _generator_index_dicts(flats::Vector{IndFlat{K}},
...
function _critical_coords(flats::Vector{IndFlat{K}}, injectives::Vector{IndInj{K}}) where {K}
...
function _signature_at(g::AbstractVector{<:Integer},
                       flats::Vector{IndFlat{K}}, injectives::Vector{IndInj{K}}) where {K}
...
function _signature_at(g::NTuple{N,<:Integer}, flats::Vector{IndFlat{K}}, injectives::Vector{IndInj{K}}) where {N,K}
    y = falses(length(flats))
    z = falses(length(injectives))
...
"""
    encode_from_flange(FG::Flange{K}; include_all_regions=true, max_regions=200_000) -> (P, H, pi)
...
- include_all_regions::Bool = true:
    Included for API compatibility with the PL encoders and higher-level code
    that wants to toggle whether "zero regions" are kept.

    For the Z^n flange encoder, the region enumeration coming from the critical
    coordinate slab decomposition is already exhaustive, so at present this flag
    does not change the output (it is accepted and ignored).
...
function encode_from_flange(FG::Flange{K};
    ...
    # NOTE: include_all_regions is currently informational for Z^n encodings.
    # We accept it so that generic "encoding front-ends" can share code between
    # Z^n and PL backends without conditional keyword logic.
    P, Hs, pi = encode_from_flanges(FG;
...
"""
    encode_from_flanges(FG1, FG2, ...; include_all_regions=true, max_regions=200_000) -> (P, Hs, pi)
...
Keyword arguments
- include_all_regions::Bool = true:
    Accepted for API compatibility with PL encoders. Currently does not change the
    output for Z^n encodings (the slab enumeration is exhaustive).
- max_regions::Int = 200_000:
    Hard cap on the number of distinct regions/signatures allowed during encoding.
"""
