...
struct ExtSpaceProjective{K}
    # Module arguments. We store both explicitly to avoid any runtime getproperty
    # aliasing (one true field set).
    #
    # Invariant: E.M === E.res.M
    M::PModule{K}
    N::PModule{K}

    # Projective resolution of M (in degrees tmin..tmax).
    res::ProjectiveResolution{K}

    # Cochain complex Hom_P(res, N) and cached basis offsets / cohomology data.
    complex::ChainComplexes.CochainComplex{K}
    offsets::Vector{Vector{Int}}
    cohom::Vector{ChainComplexes.CohomologyData{K}}

    # Cached degree bounds. For projective Ext we always have tmin == 0, but we
    # store it explicitly for consistency across ExtSpace types.
    tmin::Int
    tmax::Int
end
...
struct ExtSpaceInjective{K}
    ...
    homs::Vector{Vector{PMorphism{K}}}
    complex::CochainComplex{K}                 # Hom_P(M, res)
    cohom::Vector{ChainComplexes.HomologyData{K}}
    tmin::Int
    tmax::Int
end

# NOTE: `tmin`/`tmax` are cached fields copied from `E.complex` at construction.
# We avoid Base.getproperty/propertynames shims to keep a single, explicit field set.
...
struct ExtSpace{K}
    ...
    P2I::Vector{Vector{Matrix{K}}}  # map from projective coords to injective coords per degree
    I2P::Vector{Vector{Matrix{K}}}  # map from injective coords to projective coords per degree
    canon::ExtCanon{K}
    tmin::Int
    tmax::Int
end
...
Field names are intended to be mathematician-friendly and stable.

# Fields
- `Rop`, `A`, `B`, `C`: the input modules.
- `i`, `p`: the maps `A -> B` and `B -> C`.
- `TorA`, `TorB`, `TorC`: `Tor(Rop, A)`, `Tor(Rop, B)`, `Tor(Rop, C)` (computed up to degree `maxdeg`).
- `iH`, `pH`: induced maps on Tor groups in each degree (as matrices).
- `delta`: the connecting maps `Tor_s(Rop, C) -> Tor_{s-1}(Rop, A)` for `s = 1..maxdeg`.
- `maxdeg`: maximum degree computed.
...
See also: [`TorLongExactSequenceFirst`](@ref).
"""
