...
#
# A flange phi : (\oplus flats) -> (\oplus injectives) is stored as a scalar matrix
# `phi` with:
#   - rows = indecomposable injectives
#   - cols = indecomposable flats
#
# At a degree g, take the submatrix with the active rows/cols; dim M_g = rank(phi_g).
# (Remark 5.5, 5.11; Def. 5.14; Prop. 5.17.)
#
# This file intentionally avoids backward-compatibility shims:
#   * no Base.getproperty-based field aliases
#   * one constructor shape per type
#   * a single set of field names
# ------------------------------------------------------------------------------

using LinearAlgebra

# We keep the QQ name available in this module (used pervasively elsewhere),
# but the core data types below do not depend on the coefficient ring.
...
# ------------------------------------------------------------------------------
# Canonical constructors (single source of truth)
# ------------------------------------------------------------------------------

"""
    IndFlat(tau::Face, b::AbstractVector{<:Integer}; id::Symbol=:F) -> IndFlat

Construct an indecomposable flat label.

Arguments
---------
- `tau`: a `Face` describing the free coordinates.
- `b`: an integer vector of length `tau.n`.
- `id`: an optional label for debugging / display.

Notes
-----
We do **not** support reordered positional arguments (e.g. `IndFlat(b, tau, ...)`).
This keeps the API unambiguous and makes grepping for callsites easy.
"""
function IndFlat(tau::Face, b::AbstractVector{<:Integer}; id::Symbol = :F)
    ...
    bb = [Int(x) for x in b]
    return IndFlat(bb, tau, id)
end

"""
    IndInj(tau::Face, b::AbstractVector{<:Integer}; id::Symbol=:E) -> IndInj

Construct an indecomposable injective label.

This is the injective analogue of `IndFlat`.
"""
...
A flange presentation over Z^n with coefficient ring/type `K`.

Fields
------
- `n::Int`: ambient dimension.
- `flats::Vector{IndFlat}`: indecomposable flats.
- `injectives::Vector{IndInj}`: indecomposable injectives.
- `phi::Matrix{K}`: scalar matrix with rows indexing injectives and columns indexing flats.

The constructor enforces the convention "rows = injectives, cols = flats" and zeros
out entries forbidden by intersection constraints.
"""
struct Flange{K}
    n::Int
    ...
    function Flange{K}(n::Int,
        ...
        # Label dimension checks (fail fast, helpful errors).
        for F in flats
            length(F.b) == n || error("Flange: flat label has wrong dimension (expected n=$(n))")
            F.tau.n == n      || error("Flange: flat face has wrong dimension (expected n=$(n))")
        end
        for E in injectives
            length(E.b) == n || error("Flange: injective label has wrong dimension (expected n=$(n))")
            E.tau.n == n     || error("Flange: injective face has wrong dimension (expected n=$(n))")
        end

        # Materialize as a dense Matrix{K} once (important for performance).
        M = Matrix{K}(phi)

        # Zero out entries that are forced to be zero by intersection constraints.
        # This keeps later degree-wise rank queries fast and avoids surprising junk.
        for q in 1:m_expected, p in 1:n_expected
            if !intersects(flats[p], injectives[q])
                M[q, p] = zero(K)
            end
        end
...
"Canonical scalar matrix (1 when F_p cap E_q is nonempty, else 0)."
...
so users (and downstream code) can write `dim_at(M, ...)` uniformly.
"""
...
# ------------------ ergonomic Face constructor ------------------------------------

"""
    face(n, coords) -> Face

Ergonomic constructor for `Face` (a subset tau of {1,...,n}).
...
function face(n::Integer, coords)
    ...
    # Support tuples like (1,2) or (false,true).
    if coords isa Tuple
...
export Face, face, IndFlat, IndInj, Flange,
       canonical_matrix,
       active_flats, active_injectives, degree_matrix, dim_at,
       bounding_box
