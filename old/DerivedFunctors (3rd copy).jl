...
function Ext(res::ProjectiveResolution{QQ}, N::PModule{QQ})
    ...
    return ExtSpaceProjective{QQ}(res.M, N, res, C, offs, cohom)
end
...
Fields:
- `LES.TorA`, `LES.TorB`, `LES.TorC` are the Tor spaces.
- `LES.delta[s]` is the connecting map in degree s (for s=1..maxdeg).
"""
struct TorLongExactSequenceSecond{K}
    ...
    iH::Vector{Matrix{K}}
    pH::Vector{Matrix{K}}
    delta::Vector{Matrix{K}}  # delta[s] for s=1..maxdeg
    maxdeg::Int
end
