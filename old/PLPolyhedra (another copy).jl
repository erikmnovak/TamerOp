...
"""
    HPoly(n, A, b; poly=nothing, strict_mask=nothing, strict_eps=STRICT_EPS_QQ) -> HPoly

Construct an `HPoly` from an H-representation `A*x <= b` in exact arithmetic.

By default, all facets are treated as non-strict.

Keyword arguments
-----------------
- `poly`: optional Polyhedra object (or `nothing`).
- `strict_mask`: BitVector marking strict rows (>= constraints stored internally as <= with slack).
- `strict_eps`: exact positive slack used for strict rows.
"""
function HPoly(
    n::Int,
    A::Matrix{QQ},
    ...
    m = size(A, 1)
    if strict_mask === nothing
        strict_mask = falses(m)
    else
        length(strict_mask) == m || error("HPoly: strict_mask length mismatch")
        strict_mask isa BitVector || (strict_mask = BitVector(strict_mask))
    end
    return HPoly(n, A, b, poly, strict_mask, strict_eps)
end
...
function make_hpoly(A::AbstractMatrix, b::AbstractVector)
    ...
    return HPoly(n, Aqq, bqq; poly=poly)
end
