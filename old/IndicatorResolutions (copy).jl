...
The store still supports dictionary-like access `store[(u,v)]`.
"""
...
#
# We support both:
#   store[(u,v)]  (backwards-compatible)
#   store[u,v]    (faster; avoids allocating the tuple)
#
# Internally, hot loops should prefer store[u,v] or, even better,
# store-aligned iteration via succs/maps_to_succ.
...
@inline Base.haskey(store::CoverEdgeMapStore, e::Tuple{Int,Int})::Bool =
    haskey(store, e[1], e[2])

@inline Base.getindex(store::CoverEdgeMapStore{K,MatT}, e::Tuple{Int,Int}) where {K,MatT} =
    getindex(store, e[1], e[2])

# Structural equality: important for tests and for _same_pmodule in DerivedFunctors.
function Base.:(==)(A::CoverEdgeMapStore{K}, B::CoverEdgeMapStore{K}) where {K}
    A === B && return true
    A.nedges == B.nedges || return false
    A.preds == B.preds || return false
    A.succs == B.succs || return false

    # Each edge appears once in maps_to_succ; compare only that direction.
    @inbounds for u in 1:length(A.succs)
        Au = A.maps_to_succ[u]
        Bu = B.maps_to_succ[u]
        length(Au) == length(Bu) || return false
        for j in eachindex(Au)
            Au[j] == Bu[j] || return false
        end
    end
    return true
end
...
@inline function Base.get(store::CoverEdgeMapStore, e::Tuple{Int,Int}, default)
    haskey(store, e) ? store[e] : default
end

@inline function Base.get(f::Function, store::CoverEdgeMapStore, e::Tuple{Int,Int})
    haskey(store, e) ? store[e] : f()
end

Base.length(store::CoverEdgeMapStore) = store.nedges
Base.IteratorSize(::Type{<:CoverEdgeMapStore}) = Base.HasLength()
Base.eltype(::Type{CoverEdgeMapStore{K,MatT}}) where {K,MatT} = Pair{Tuple{Int,Int},MatT}
Base.valtype(::Type{CoverEdgeMapStore{K,MatT}}) where {K,MatT} = MatT
Base.keytype(::Type{CoverEdgeMapStore{K,MatT}}) where {K,MatT} = Tuple{Int,Int}

# deterministic iteration: u-major, then v-major
function Base.iterate(store::CoverEdgeMapStore, state=(1, 1))
    u, j = state
    n = length(store.succs)
    while u <= n
        su = @inbounds store.succs[u]
        if j <= length(su)
            v = @inbounds su[j]
            A = @inbounds store.maps_to_succ[u][j]
            return ((u, v) => A), (u, j + 1)
        end
        u += 1
        j = 1
...
Build a store aligned with the cover graph of `Q`. Missing cover-edge maps are
filled with the zero map of the appropriate shape.

`edge_maps` may be a dict or any mapping supporting `haskey` and `getindex`.
"""
function CoverEdgeMapStore{K,MatT}(
    ...
    for v in 1:n
        ...
        @inbounds for i in eachindex(pv)
            u = pv[i]
            key = (u, v)
            A = haskey(edge_maps, key) ? convert(MatT, edge_maps[key]) :
                                         _zero_edge_map(MatT, K, dv, dims[u])
...
# Many parts of the library (and older notebooks/tests) refer to the codomain of a
# morphism as `f.codom`. Internally the field is stored as `f.cod`.
# Provide `codom` as a read-only alias so both spellings work.
function Base.getproperty(f::PMorphism{K}, s::Symbol) where {K}
    if s === :codom
        return getfield(f, :cod)
    end
    return getfield(f, s)
end

function Base.propertynames(::PMorphism{K}, private::Bool=false) where {K}
    if private
        return (:dom, :cod, :codom, :comps)
    else
        return (:dom, :cod, :codom)
    end
end
