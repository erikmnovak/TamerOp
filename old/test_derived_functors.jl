...
@testset "Minimality diagnostics for projective/injective resolutions" begin
    ...
    resP = PM.projective_resolution(S1; maxlen=4)
    ...
    # The explicit "minimal_*" wrapper should succeed and return a minimal resolution.
    resPmin = PM.minimal_projective_resolution(S1; maxlen=4)
    ...
    resI = PM.injective_resolution(S1; maxlen=4)
    ...
    resImin = PM.minimal_injective_resolution(S1; maxlen=4)
...
@testset "Yoneda associativity sanity check (B3, degree 3 is nonzero)" begin
    ...
    E03 = PM.Ext(S0, S123; maxdeg=3)
    @test PM.dim(E03, 3) == 1

    # Degree-1 generators on the cover chain:
    #   {} -> {1} -> {1,2} -> {1,2,3}
    E23 = PM.Ext(S12, S123; maxdeg=3)
    E12 = PM.Ext(S1,  S12;  maxdeg=3)
    E01 = PM.Ext(S0,  S1;   maxdeg=3)
    ...
    E13 = PM.Ext(S1,  S123; maxdeg=3)
    E02 = PM.Ext(S0,  S12;  maxdeg=3)
    @test PM.dim(E13, 2) == 1
    @test PM.dim(E02, 2) == 1
...
@testset "Connecting homomorphisms: split exact sequences give zero maps" begin
    ...
    t = 2
    resM = PM.projective_resolution(S1; maxlen=t+1)   # i.e. maxlen=3
    EMA = PM.Ext(resM, A)
    EMB = PM.Ext(resM, B)
    EMC = PM.Ext(resM, C)
...
@testset "Connecting homomorphisms: nonsplit extension gives nonzero delta" begin
    ...
    resM = PM.projective_resolution(S1; maxlen=2)
    EMA = PM.Ext(resM, S2)
    EMB = PM.Ext(resM, I12)
    EMC = PM.Ext(resM, S1)
    ...
    les = PM.ExtLongExactSequenceSecond(S1, S2, I12, S1, i, p; maxdeg=0)
    @test EX.rankQQ(les.deltaH[1]) == 1
    @test Matrix(les.deltaH[1]) == Matrix(delta0)
end
...
@testset "Sparse assembly replacements (dense->sparse removed)" begin
    ...
    M = IR.PModule{QQ}(P, dims, edge_maps)
...
@testset "DerivedFunctors: downset postcompose coefficient solver" begin
    ...
    X  = IR.PModule{QQ}(Q, [3], Dict{Tuple{Int, Int}, Matrix{QQ}}())
    E  = IR.PModule{QQ}(Q, [3], Dict{Tuple{Int, Int}, Matrix{QQ}}())
    Ep = IR.PModule{QQ}(Q, [2], Dict{Tuple{Int, Int}, Matrix{QQ}}())

    F1 = Matrix{QQ}(I, 3, 3)
    G1 = QQ[1 2 3;
            4 5 6]

    f = IR.PMorphism{QQ}(X, E,  [F1])
    g = IR.PMorphism{QQ}(X, Ep, [G1])
