...
# NOTE: We intentionally avoid adapter helpers like _edge_maps_haskey/_edge_maps_getindex.
# Construction supports either an existing CoverEdgeMapStore (canonical) or a tuple-keyed dict,
# via explicit dispatch in the constructors below.
...
function CoverEdgeMapStore{K,MatT}(
    Q::FinitePoset,
    dims::Vector{Int},
    edge_maps::CoverEdgeMapStore;
    cache::Union{Nothing,CoverCache}=nothing,
    check_sizes::Bool=true,
) where {K,MatT<:AbstractMatrix{K}}

    # Explicit dispatch: edge_maps is already a CoverEdgeMapStore, so use
    # haskey(edge_maps, u, v) and edge_maps[u, v] directly.
...
function CoverEdgeMapStore{K,MatT}(
    ...
    # Explicit dispatch: expect tuple keys (u,v) in edge_maps.
    ...
    for v in 1:n
        ...
        @inbounds for i in eachindex(pv)
            u = pv[i]
            key = (u, v)
            if haskey(edge_maps, key)
                A = convert(MatT, edge_maps[key])
            else
                A = _zero_edge_map(MatT, K, dv, dims[u])
            end
