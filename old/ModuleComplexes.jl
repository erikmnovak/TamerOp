...
using ..CoreModules: QQ, _append_scaled_triplets!
...
using ..IndicatorResolutions:
    PModule, PMorphism,
    id_morphism, kernel_with_inclusion, image_with_inclusion,
    _cokernel_module,
...
using ..ChainComplexes:
    DoubleComplex, CochainComplex, CochainMap, total_complex, cohomology_data, spectral_sequence,
...
# We extend the common `dim` API (re-exported by PosetModules) for HyperExtSpace/HyperTorSpace.
import ..DerivedFunctors: dim
...
"""
    ModuleCochainComplex(terms, diffs, tmin, tmax; check=true)

Positional-endpoint overload for `ModuleCochainComplex`.

This is a thin wrapper around the keyword constructor

    ModuleCochainComplex(terms, diffs; tmin=tmin, check=check)

and exists for two reasons:

1. Backward compatibility with older call sites that passed both endpoints.
2. A mathematician-friendly style where you want to record both `tmin` and
   `tmax` explicitly at the call site.

The endpoints must be consistent with the number of terms:
...
function ModuleCochainComplex(
    ...
    expected = tmin + length(terms) - 1
    if tmax != expected
        error(
            "ModuleCochainComplex: inconsistent endpoints: expected tmax = " *
            "tmin + length(terms) - 1 = " * string(expected) *
            ", got tmax = " * string(tmax) * "."
        )
    end
    return ModuleCochainComplex(terms, diffs; tmin=tmin, check=check)
end
...
function RHomComplex(
    ...
    resN = (resN === nothing) ? injective_resolution(N; maxlen = maxlen) : resN
        ...
        Threads.@threads for slot in 1:nT
            for idx in slot:nT:(na * nb)
                ...
                if ib == nb
                    ...
                    q = ib - 1
                    dh[ia, ib] = _postcompose_matrix(resN.diffs[q + 1], homs[ia, ib], homs[ia, ib + 1])
                end
            end
        end
    else
...
function rhom_map_first(
    ...
    resN = isnothing(resN) ? injective_resolution(N; maxlen=maxlen) : resN
...
# Keep this underscored wrapper for internal callers/backwards compatibility.
function _lift_to_injective_resolution_chainmap(g::PMorphism{QQ},
                                                resN::InjectiveResolution{QQ},
                                                resNp::InjectiveResolution{QQ};
                                                upto::Union{Nothing, Int}=nothing,
                                                check::Bool=true)
    return lift_injective_chainmap(g, resN, resNp; upto=upto, check=check)
end
...
function rhom_map_second(
    ...
    phis = _lift_to_injective_resolution_chainmap(g, Rsrc.resN, Rtgt.resN; check=check)
...
function DerivedTensorComplex(
    ...
    resR = projective_resolution(Rop; maxlen = maxlen, check = check)
...
function dim(H::HyperTorSpace, n::Int)
    t = -n
    if t < H.T.tot.tmin || t > H.T.tot.tmax
        return 0
    end
    return H.cohom[t - H.T.tot.tmin + 1].dimH
end
...
function derived_tensor_map_first(
    ...
    for t in tmin:tmax
        ...
        for (key, col_off) in off_dom
            ...
            A, p = key
            a = -A
            (a < 0 || a > Tdom.resR.maxlen) && continue
            (a < 0 || a > Tcod.resR.maxlen) && continue
...
function derived_tensor_map_second(
    ...
    for t in tmin:tmax
        ...
        for (key, col_off) in off_src
            ...
            A, p = key
            a = -A
            (a < 0 || a > Tsrc.resR.maxlen) && continue
