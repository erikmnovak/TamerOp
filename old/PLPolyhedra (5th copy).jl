...
# Canonical constructor with explicit strictness controls.
#
# By default we assume all inequalities are non-strict (">=" style). If you
# need strict rows, pass `strict_mask` and (optionally) `strict_eps`.
function HPoly(
    n::Int,
    A::Matrix{QQ},
    b::Vector{QQ};
    ...
    m = size(A, 1)
    length(b) == m || error("b has length $(length(b)), expected $m")
    length(strict_mask) == m || error("strict_mask has length $(length(strict_mask)), expected $m")
