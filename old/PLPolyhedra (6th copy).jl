...
# Canonical convenience constructor.
#
# We keep a single constructor *shape* by using keywords for optional data
# (poly, strict rows). This avoids maintaining multiple positional constructor
# variants that drift over time.
function HPoly(
    n::Int,
    A::Matrix{QQ},
    b::Vector{QQ};
    poly::Any=nothing,
    strict_mask::BitVector=falses(size(A, 1)),
    strict_eps::QQ=STRICT_EPS_QQ,
)
    m = size(A, 1)
    size(A, 2) == n || error("A must have n columns")
    length(b) == m || error("b must have one entry per row of A")
    length(strict_mask) == m || error("strict_mask must have one entry per row of A")
