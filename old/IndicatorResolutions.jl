...
function direct_sum(A::PModule{K}, B::PModule{K}) where {K}
    Q = A.Q
    @assert B.Q === Q
    dims = [A.dims[i] + B.dims[i] for i in 1:Q.n]
    edge = Dict{Tuple{Int,Int}, Matrix{K}}()
    for (u,v) in cover_edges(Q)
        Au = A.edge_maps[(u,v)]
        Bu = B.edge_maps[(u,v)]
        ...
        M = zeros(K, av + bv, au + bu)
        if av > 0 && au > 0
            M[1:av, 1:au] .= Au
        end
        if bv > 0 && bu > 0
            M[av+1:av+bv, au+1:au+bu] .= Bu
        end
        edge[(u,v)] = M
    end
...
function direct_sum_with_maps(A::PModule{K}, B::PModule{K}) where {K}
    S = direct_sum(A,B)
    Q = A.Q
    iA_comps = Vector{Matrix{K}}(undef, Q.n)
    iB_comps = Vector{Matrix{K}}(undef, Q.n)
    pA_comps = Vector{Matrix{K}}(undef, Q.n)
    pB_comps = Vector{Matrix{K}}(undef, Q.n)
    for u in 1:Q.n
        a = A.dims[u]; b = B.dims[u]
        iA = zeros(K, a+b, a)
        iB = zeros(K, a+b, b)
        pA = zeros(K, a, a+b)
        pB = zeros(K, b, a+b)
        if a > 0
            iA[1:a, 1:a] .= Matrix{K}(I, a, a)
            pA[1:a, 1:a] .= Matrix{K}(I, a, a)
        end
        if b > 0
            iB[a+1:a+b, 1:b] .= Matrix{K}(I, b, b)
            pB[1:b, a+1:a+b] .= Matrix{K}(I, b, b)
        end
        iA_comps[u] = iA
        iB_comps[u] = iB
        pA_comps[u] = pA
    ...
    return (
        S,
        PMorphism{K}(A,S,iA_comps),
        PMorphism{K}(B,S,iB_comps),
        PMorphism{K}(S,A,pA_comps),
        PMorphism{K}(S,B,pB_comps),
    )
end
...
"""
function image_with_inclusion(f::PMorphism{QQ})
    M = f.dom
    N = f.cod
    Q = M.Q
    @assert N.Q === Q

    # Choose basis for Im(f_u) at each vertex u.
    bases = Vector{Matrix{QQ}}(undef, Q.n)
    dims = zeros(Int, Q.n)
    for u in 1:Q.n
        B = colspaceQQ(f.comps[u])
        bases[u] = B
        dims[u] = size(B, 2)
    end

    # Structure maps on image: express N(u<=v)*basis[u] in basis[v].
    edge = Dict{Tuple{Int,Int}, Matrix{QQ}}()
    for (u,v) in cover_edges(Q)
        Mu = bases[u]
        Mv = bases[v]
        if size(Mu,2) == 0
            edge[(u,v)] = zeros(QQ, size(Mv,2), 0)
            continue
        end
        T = N.edge_maps[(u,v)] * Mu
        if size(Mv,2) == 0
            edge[(u,v)] = zeros(QQ, 0, size(Mu,2))
        else
            edge[(u,v)] = solve_fullcolumnQQ(Mv, T)
        end
    end

    Im = PModule{QQ}(Q, collect(dims), edge)
    iota_comps = [bases[u] for u in 1:Q.n]
    iota = PMorphism{QQ}(Im, N, iota_comps)
    return Im, iota
end
...
function _direct_sum_many_with_maps(mods::AbstractVector{<:PModule{K}}) where {K}
    ...
    for M in mods
        if M.Q !== Q
            error("_direct_sum_many_with_maps: all modules must live on the same poset")
        end
    end

    # Precompute block offsets at each vertex.
    offsets = [zeros(Int, m + 1) for _ in 1:Q.n]
    Sdims = zeros(Int, Q.n)
    for u in 1:Q.n
        off = 0
        offsets[u][1] = 0
    ...
    # Edge maps are block-diagonal (one diagonal block per summand).
    edge_maps = Dict{Tuple{Int,Int}, Matrix{K}}()
    for (e, _) in mods[1].edge_maps
        u, v = e
        A = zeros(K, Sdims[v], Sdims[u])
        for i in 1:m
            du = mods[i].dims[u]
            dv = mods[i].dims[v]
            if du == 0 || dv == 0
            ...
            c0 = offsets[u][i] + 1
            c1 = offsets[u][i + 1]
            A[r0:r1, c0:c1] .= mods[i].edge_maps[e]
        end
        edge_maps[e] = A
    end
    S = PModule{K}(Q, collect(Sdims), edge_maps)
    ...
    for i in 1:m
        ...
        for u in 1:Q.n
            ...
            # Injection mods[i] -> S: identity block in the appropriate rows.
            Iu = zeros(K, Su, du)
            if du > 0
                r0 = offsets[u][i] + 1
                r1 = offsets[u][i + 1]
                Iu[r0:r1, :] .= Matrix{K}(I, du, du)
            end
            push!(inj_comps, Iu)

            # Projection S -> mods[i]: identity block in the appropriate columns.
            Pu = zeros(K, du, Su)
            if du > 0
                c0 = offsets[u][i] + 1
                c1 = offsets[u][i + 1]
                Pu[:, c0:c1] .= Matrix{K}(I, du, du)
            end
            push!(prj_comps, Pu)
        end
        push!(injections, PMorphism{K}(mods[i], S, inj_comps))
        push!(projections, PMorphism{K}(S, mods[i], prj_comps))
    end
