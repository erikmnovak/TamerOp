...
struct LongExactSequence{K}
    ...
    deltaH::Vector{Matrix{K}}  # H^t(Cone) -> H^{t+1}(C)
end
...
function long_exact_sequence(tri::DistinguishedTriangle{QQ})
    ...
    for (k, t) in enumerate(tmin:tmax)
        if t + 1 > tmax
            push!(deltaH, zeros(QQ, 0, Hcone[k].dimH))
            continue
        end
        k_next = (t + 1) - tmin + 1
        Hsh = HCshift[k]
        ...
        push!(deltaH, iso * pH[k])
    end

    return LongExactSequence{QQ}(tri, tmin, tmax, HC, HD, Hcone, HCshift, fH, iH, pH, deltaH)
end
