...
# ------------------------------------------------------------------------------
# Mathematician-friendly constructors for IndFlat / IndInj.
#
# Why these exist:
# - IndFlat{K} / IndInj{K} are parametric in K, but K is not inferable from the
#   fields (b, tau, id), so calling IndFlat(...) without specifying {K} fails
#   unless we provide an explicit default.
# - Some tests and older client code expect to be able to write:
#       IndFlat(tau, b)     and     IndInj(tau, b)
#   where tau is a Face and b is an integer vector.
#
# The unparameterized constructors below default to K = QQ.
# We also provide reorderings for the parametric constructors IndFlat{K} and
# IndInj{K} so that IndFlat{K}(tau, b) works as well.
# ------------------------------------------------------------------------------

"""
    IndFlat(tau::Face, b::AbstractVector{<:Integer}; id=:F) -> IndFlat{QQ}
    IndFlat(b::AbstractVector{<:Integer}, tau::Face; id=:F) -> IndFlat{QQ}

Convenience constructors for an indecomposable flat label.

The underlying type `IndFlat{K}` is parametric in the coefficient type `K`,
but `K` cannot be inferred from `(b, tau, id)`. Therefore the *unparameterized*
constructor defaults to `K = QQ`.

Examples
--------
    tau = face(2, [2])          # tau = {2}
    F1  = IndFlat(tau, [0, 0])  # id defaults to :F
    F2  = IndFlat([0, 0], tau; id=:F2)
"""
function IndFlat(tau::Face, b::AbstractVector{<:Integer}; id::Symbol = :F)
    length(b) == tau.n || error("IndFlat: dimension mismatch (length(b) != tau.n)")
    bb = [Int(x) for x in b]
    return IndFlat{QQ}(bb, tau, id)
end

IndFlat(b::AbstractVector{<:Integer}, tau::Face; id::Symbol = :F) =
    IndFlat(tau, b; id=id)

"""
    IndInj(tau::Face, b::AbstractVector{<:Integer}; id=:E) -> IndInj{QQ}
    IndInj(b::AbstractVector{<:Integer}, tau::Face; id=:E) -> IndInj{QQ}

Convenience constructors for an indecomposable injective label, defaulting to
`K = QQ` for the same reason as `IndFlat`.
"""
function IndInj(tau::Face, b::AbstractVector{<:Integer}; id::Symbol = :E)
    length(b) == tau.n || error("IndInj: dimension mismatch (length(b) != tau.n)")
    bb = [Int(x) for x in b]
    return IndInj{QQ}(bb, tau, id)
end

IndInj(b::AbstractVector{<:Integer}, tau::Face; id::Symbol = :E) =
    IndInj(tau, b; id=id)

# -- Parametric reorderings (when the user explicitly wants K != QQ) ------------

function IndFlat{K}(tau::Face, b::AbstractVector{<:Integer}; id::Symbol = :F) where {K}
    length(b) == tau.n || error("IndFlat{$(K)}: dimension mismatch (length(b) != tau.n)")
    bb = [Int(x) for x in b]
    return IndFlat{K}(bb, tau, id)
end

IndFlat{K}(b::AbstractVector{<:Integer}, tau::Face; id::Symbol = :F) where {K} =
    IndFlat{K}(tau, b; id=id)

function IndInj{K}(tau::Face, b::AbstractVector{<:Integer}; id::Symbol = :E) where {K}
    length(b) == tau.n || error("IndInj{$(K)}: dimension mismatch (length(b) != tau.n)")
    bb = [Int(x) for x in b]
    return IndInj{K}(bb, tau, id)
end

IndInj{K}(b::AbstractVector{<:Integer}, tau::Face; id::Symbol = :E) where {K} =
    IndInj{K}(tau, b; id=id)
...
# Property aliases matching common notation in the paper:
#   FG.F   == FG.flats
#   FG.E   == FG.injectives
#   FG.phi == FG.Phi
function Base.getproperty(FG::Flange, s::Symbol)
    if s === :F
        return getfield(FG, :flats)
    elseif s === :E
        return getfield(FG, :injectives)
    elseif s === :phi
        return getfield(FG, :Phi)
    else
        return getfield(FG, s)
    end
end

function Base.propertynames(FG::Flange, private::Bool=false)
    base = fieldnames(typeof(FG))
    return private ? (base..., :F, :E, :phi) : (base..., :F, :E, :phi)
end
...
# ---------------- short constructors (default field QQ) -------------------------
# The indecomposable labels IndFlat{K} / IndInj{K} are parametric in K, but K is
# not inferable from the fields, so user-facing constructors should pick QQ by default.

"Convenience constructor: Flat(id, b, tau_mask_or_face) -> IndFlat{QQ}."
function Flat(id::Symbol, b::AbstractVector{<:Integer}, tau_mask::AbstractVector{Bool})
    n = length(b)
    length(tau_mask) == n || error("Flat: tau mask must have length n=$(n)")
    bb = [Int(x) for x in b]
    return IndFlat{QQ}(bb, Face(n, tau_mask), id)
end

function Flat(id::Symbol, b::AbstractVector{<:Integer}, tau::Face)
    length(b) == tau.n || error("Flat: dimension mismatch (length(b) != tau.n)")
    bb = [Int(x) for x in b]
    return IndFlat{QQ}(bb, tau, id)
end

"Convenience constructor: Injective(id, b, tau_mask_or_face) -> IndInj{QQ}."
function Injective(id::Symbol, b::AbstractVector{<:Integer}, tau_mask::AbstractVector{Bool})
    n = length(b)
    length(tau_mask) == n || error("Injective: tau mask must have length n=$(n)")
    bb = [Int(x) for x in b]
    return IndInj{QQ}(bb, Face(n, tau_mask), id)
end

function Injective(id::Symbol, b::AbstractVector{<:Integer}, tau::Face)
    length(b) == tau.n || error("Injective: dimension mismatch (length(b) != tau.n)")
    bb = [Int(x) for x in b]
    return IndInj{QQ}(bb, tau, id)
end

"""
    flat(b, tau_mask_or_face; id=:F) -> IndFlat{QQ}

Lowercase convenience constructor for an indecomposable flat label.
This is a thin wrapper around `Flat(id, b, ...)` and exists for ergonomic test code:
    FZ.flat([0,0], [false,true])
"""
flat(b::AbstractVector{<:Integer}, tau_mask::AbstractVector{Bool}; id::Symbol=:F) =
    Flat(id, b, tau_mask)

flat(b::AbstractVector{<:Integer}, tau::Face; id::Symbol=:F) =
    Flat(id, b, tau)


"""
    inj(b, tau_mask_or_face; id=:E) -> IndInj{QQ}

Lowercase convenience constructor for an indecomposable injective label.
Thin wrapper around `Injective(id, b, ...)`:
    FZ.inj([0,0], [false,true])
"""
inj(b::AbstractVector{<:Integer}, tau_mask::AbstractVector{Bool}; id::Symbol=:E) =
    Injective(id, b, tau_mask)

inj(b::AbstractVector{<:Integer}, tau::Face; id::Symbol=:E) =
    Injective(id, b, tau)
...
export Face, face, IndFlat, IndInj, Flat, Injective, flat, inj, Flange, canonical_matrix,
